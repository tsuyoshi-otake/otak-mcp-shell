...
### Read
指定されたファイルの内容を読み取ります。部分読み取りも可能です。
- パラメータ: `path` (必須), `offset` (省略可), `limit` (省略可)
- 戻り値: ファイルの内容（テキスト）
...
- パラメータ: `oldPath` (必須), `newPath` (必須)
- 戻り値: 成功メッセージ

### Copy
ファイルまたはディレクトリを別の場所にコピーします。
- パラメータ: `source` (必須), `destination` (必須)
- 戻り値: 成功メッセージ

### Stat
ファイルの詳細情報を取得します。
- パラメータ: `path` (必須)
- 戻り値: ファイルサイズ、タイムスタンプ、権限などのメタデータ

### Tail
ファイルの末尾N行を取得します（ログファイルに便利）。
- パラメータ: `path` (必須), `lines` (省略可、デフォルト10)
- 戻り値: ファイルの末尾行

### Edit
ファイルの部分的な更新を行います（find-and-replace）。
- パラメータ: `path` (必須), `old_text` (必須), `new_text` (必須)
- 戻り値: 成功メッセージ

### MultiEdit
1つのファイルに対して複数のfind-and-replace操作を実行します。
- パラメータ: `path` (必須), `edits` (必須: old_text, new_textの配列)
- 戻り値: 成功メッセージ

### Glob
高性能なファイル検索（ワイルドカード対応）。
- パラメータ: `pattern` (必須), `recursive` (省略可)
- 戻り値: マッチするファイルのパス一覧

### Grep
ファイル内のテキスト検索（正規表現対応）。
- パラメータ: `pattern` (必須), `filePattern` (省略可)
- 戻り値: マッチした行とファイル名

### Search
ファイルパターンマッチング（globパターン、修更新日時順）。
- パラメータ: `pattern` (必須)
- 戻り値: マッチするファイルのパス一覧
